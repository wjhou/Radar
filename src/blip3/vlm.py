    ...
    def _prepare_inputs_for_forward(
        ...
        for i in range(B):
            ...
            for img_num, img_idx in enumerate(image_token_idxs):
                ...
                if self.image_aspect_ratio == "anyres" and False:
...
class KosmosInstruct(VLMWithLanguageStream):
    def __init__(
        ...
        # self.residual = nn.Linear(lang_embedding_dim, lang_embedding_dim)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if vision_x is not None:
            ...
            # vision_tokens = vision_tokens + self.residual(vision_tokens)
        else:
            vision_tokens = None
    ...
    def generate(
        ...
        **kwargs,
        ...
        if "max_length" in kwargs:
            kwargs["max_length"] += new_inputs["inputs_embeds"].size(1)
        if Version(transformers.__version__) >= Version("4.41.1"):
            output = self.lang_model.generate(
                **new_inputs,
                ...
                logits_processor=[
                    EosLogitProcessor(
                        eos_token_id=self.lang_model.config.eos_token_id,
                        end_token_id=32007,
                    )
                ],
                **kwargs,
